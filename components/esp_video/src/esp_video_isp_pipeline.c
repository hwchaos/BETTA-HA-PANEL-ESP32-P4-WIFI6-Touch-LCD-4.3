...
/* Local patch (Cpt_Kirk) - see docs/WAVESHARE-7B-PORT.md, "ISP log flood".
 * Upstream turns on this component's per-frame ISP statistics dump and forces
 * its own log tag to DEBUG whenever the build allows DEBUG at all
 * (LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG).  This application needs
 * CONFIG_LOG_MAXIMUM_LEVEL=VERBOSE so its runtime verbosity selector can open
 * every tag, which silently made that true and produced ~200 debug lines per
 * second (~60 per camera frame) on a panel where the ISP task shares core 0 with
 * IDLE0.  The task then stayed busy past the task-watchdog window, the watchdog
 * reset the panel in a loop and took HTTP, OTA and the log down with it.  Both
 * are opt-in now; the application owns the tag level (main/diag/system_log.c
 * pins the ISP tag to WARN) and the selector can be raised without re-flashing. */
#ifndef ESP_VIDEO_ISP_VERBOSE_LOG
#define ESP_VIDEO_ISP_VERBOSE_LOG 0
#endif
